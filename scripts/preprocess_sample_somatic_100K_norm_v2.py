...
required_args.add_argument("-dt", "--depth_tumor", help="Please provide the exon-wise readdepth path.", required=True)
required_args.add_argument("-dn", "--depth_normal", help="Please provide the exon-wise readdepth path.", required=True)
...
required_args.add_argument("-gc", "--gc", help="Whether using gc correction", required=True)
required_args.add_argument("-methy", "--methy", help="Whether using methylation information", required=True)
...
if args.downsample:
    ...
    exon_wise_readdepth_files = [file for file in listdir(readdepths_path_t) if not file.startswith('.') and file.endswith(f'.{down}.bam.txt')]
    ...
    exon_wise_readdepth_files = [file for file in listdir(readdepths_path_t) if not file.startswith('.')]
...
label_files.sort()
if args.menu:
    menu = pd.read_csv(args.menu, sep='\s+', names=['ID','WGS', 'WGBS_tumor', 'WGBS_normal'])
    print(menu)
binsize = 100000
depthsize = 100
...
for wgbs_t_file in exon_wise_readdepth_files:
    ...
    if args.menu:
        if sample_name in menu['WGBS_tumor'].values.astype(str):  # 检查 sample_name 是否在 'WGBS' 列中
            wgs_name = menu.loc[menu['WGBS_tumor'] == sample_name, 'WGS'].values[0]   # 选择对应行的 'WGS' 列数据
