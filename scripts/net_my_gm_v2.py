...
class ChromosomeEmbedding(nn.Module):
    ...
    def forward(self, tensor, chr):
        ...
        #print(chr_mat_all, chr_mat_all.shape)
        return chr_mat_all
...
class CNVcaller(nn.Module):
    def __init__(self, bin_size, patch_size, depth, embed_dim, num_class, channels = 1):
        super().__init__()
        assert bin_size % patch_size == 0, 'Image dimensions must be divisible by the patch size.'
    ...
    def forward(self, data, mask):
        ...
        #print('3 x: ', np.shape(x))
        #print('pos_emb: ', np.shape(self.pos_emb(x, strt, ends)))
        #print('chr_emb: ', np.shape(self.chr_emb(x, chrs)))
        x += self.pos_emb(x, strt, ends)
        
        x += self.chr_emb(x, chrs)
        #x += self.chr_emb2(x, chr)
